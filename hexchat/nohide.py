...
__module_description__ = "Does *not* hide Part/Leave Message of users " \
    "that have interacted with the channel."
...
def nohide(word, word_eol, userdata):
    ...
    hexchat.emit_print('Generic Message', 'NoHide',
                       '{} Users in {} Channels'.format(len(nclist),
                                                        len(chanlist)))
    ...
    for chan in chanlist:
        if in_channel_list(chan):
            ...
            hexchat.emit_print('Generic Message', leftpart, '({}) '.format(
                len(nicklist)) + (', '.join(nicklist)))
